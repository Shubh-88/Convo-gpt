...
res.clearCookie(COOKIE_NAME,{
    ...
    domain:"localhost",
     ...
     res.cookie(COOKIE_NAME,token,{              // It is used to send cookies from backend to user 
        path:"/",
        domain:"localhost",
        expires,
        httpOnly:true,
...
export const UsersLogin = async(req:Request,res:Response,next:NextFunction) =>{
    ...
    if(!user) return res.status(401).send("User not Registered");    //A 401 status code in HTTP means "Unauthorized."
